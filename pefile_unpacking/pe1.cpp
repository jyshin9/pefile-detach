...
int main(int argc, char* argv[]) {
	...
	//dosHeader 분리
	FILE *dosHeader_fp = fopen("dosheader.bin", "wb"); //쓰기전용 이진파일 dosheader.bin을 가리키는 파일 포인터를 반환하여 변수 dosHeader_fp에 대입
	...
	FILE* ntHeader_fp = fopen("ntheader.bin", "wb"); 
	fwrite(&ntHeader, sizeof(IMAGE_NT_HEADERS), 1, ntHeader_fp);
	fclose(ntHeader_fp);

	FILE* sectionHeaders_fp = fopen("sectionheaders.bin", "wb");
	...
	printf("Detached headers saved to dosheader.bin, ntheader.bin, and sectionheaders.bin.\n");
